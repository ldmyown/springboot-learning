...
public class UpLoadClient {
    ...
    private static Logger logger = LoggerFactory.getLogger(UpLoadClient.class);
    ...
    public void uploadFile(String path) {  
        if(path == null) {
            logger.error("上传文件的路径不能为null");
            return;
        }  
        File file = new File(path);
        if (!file.exists()) {
            logger.error(file.getName() + "不存在");
            return;
        }
        if (!file.canRead()) {
            logger.error(file.getName() + "不可读");
            return;
        }  
